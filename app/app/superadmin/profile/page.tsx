...
export default function SuperadminProfilePage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setSaved(false);
    try {
      const updated = await updateStaffProfile("superadmin", { fullName, phone });
      setProfile(updated);
      setSaved(true);
    } catch (error) {
  ...
  return (
    <SuperadminLayout active="profile">
      <main className="max-w-[1190px] mx-auto px-6 pt-20 pb-2 max-[650px]:px-4 max-[650px]:pt-[68px] max-[650px]:pb-2 min-h-[calc(100vh-76px)]">
        <div className="flex justify-between items-end gap-5 mb-[30px] max-[780px]:items-start max-[780px]:flex-col">
          <div>
            ...
            <p className="mt-[9px] text-muted text-sm">Full workspace access details.</p>
          </div>
          <span className="px-[10px] py-[7px] rounded-2xl text-white bg-brand text-[10px] font-bold">Superadmin</span>
        </div>
        ...
        {isLoading ? (
          ...
          <form className="p-6 border border-[#e1e9e4] rounded-[10px] bg-white p-[29px] max-[500px]:px-4 max-[500px]:py-[19px]" onSubmit={save}>
            <div className="flex items-center gap-[14px] pb-[25px] border-b border-[#edf1ee] max-[780px]:items-start max-[780px]:flex-wrap">
              <span className="grid place-items-center w-[58px] h-[58px] rounded-full text-white bg-brand text-base font-bold">{initialsFor(profile.fullName ?? profile.userId)}</span>
              <div className="flex-1">
                <h2 className="m-0 leading-none font-display font-bold text-xl">{profile.fullName || profile.userId}</h2>
                <p className="m-0 mt-[6px] leading-none text-[#8b9992] text-[10px]">Superadmin since {memberSince(profile.joinedAt)} · {profile.email}</p>
              </div>
            </div>
