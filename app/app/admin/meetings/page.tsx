...
function ManageMeetingModal({ meetingId, onClose, onUpdated, onCancelled }: { meetingId: string; onClose: () => void; onUpdated: (meeting: Meeting) => void; onCancelled: (meetingId: string) => void }) {
	const { recipients, isLoading: isLoadingRecipients, error: recipientsError } = useMeetingRecipients("admin");
		...
		async function load() {
			try {
				const [meetings, shares] = await Promise.all([
					listMeetings("admin"),
					getMeetingRecipients("admin", meetingId),
				]);
				if (cancelled) return;
				setSelection(shares);
	...
	async function submit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const updated = await updateMeeting("admin", meetingId, {
				title: form.title,
				scheduledAt,
			...
			if (recipientsChanged) {
				await setMeetingRecipients("admin", meetingId, selection);
...
function AdminMeetingsPageContent() {
	...
	function openManage(meetingId: string) {
		router.push(`/admin/meetings?meetingId=${meetingId}`);
	}
