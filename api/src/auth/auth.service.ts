...
export class AuthService {
  ...
  private async getProfile(identifier: string) {
    ...
    const { data: profileById, error: idError } = await client
      ...
      .maybeSingle();
    if (idError)
      throw new InternalServerErrorException('Unable to look up the user.');
  ...
  private async sessionResult(
    ...
    if (error)
      throw new InternalServerErrorException(
        'Unable to look up the user profile.',
      );
    if (!profile) throw new UnauthorizedException('User profile not found.');
    return {
      accessToken,
      refreshToken,
      user: { id: profile.id, userId: profile.user_id, role: profile.role },
    };
  }
  ...
  async authenticate(accessToken: string) {
    ...
    const { data: profile, error: profileError } = await client
      ...
      .maybeSingle();
    if (profileError)
      throw new InternalServerErrorException('Unable to verify your role.');
    if (!profile) throw new UnauthorizedException('User profile not found.');
    return { user, profile };
  }
  async login(userId: string, password: string, meta?: RequestMeta) {
    if (!userId || !password)
    ...
    if (!profile) {
      await this.audit.log({
        ...
        entityType: 'auth',
        newData: { attemptedUserId: userId },
  ...
  async logout(accessToken?: string, meta?: RequestMeta) {
  ...
  async createUser(
    ...
    if (
      ...
      !input.email ||
      !input.password ||
      !['admin', 'member', 'accountant'].includes(input.role)
    ) {
      throw new BadRequestException(
        'userId, email, password, and a valid role are required.',
      );
    }
    ...
    if (!requesterProfile || requesterProfile.role !== routeRole) {
      throw new UnauthorizedException(
        'You are not allowed to use this user-creation route.',
      );
    }
    if (
      routeRole === 'admin' &&
      !['member', 'accountant'].includes(input.role)
    ) {
      throw new UnauthorizedException(
        'Admins can only create members or accountants.',
      );
    }
    ...
    if (createError || !created.user) {
      throw new BadRequestException(
        createError?.message ?? 'Unable to create the user.',
      );
    }
    ...
    const { error: profileError } = await client.from('profiles').insert({
      ...
      full_name: input.fullName?.trim() || userId,
    ...
    if (profileError) {
      ...
      throw new BadRequestException(
        profileError.code === '23505'
          ? 'That user ID or email is already in use.'
    ...
    await this.audit.log({
      actor: { userId: requester.id },
      action: 'user.created',
      entityType: 'profile',
      entityId: created.user.id,
      newData: { userId, role: input.role, email },
      ipAddress: meta?.ipAddress,
      userAgent: meta?.userAgent,
    });
